...
本次审查确认自动化测试与静态检查通过，并已建立 `v0.9.12-baseline` Git 基线。版本号已统一从 `ailive.__version__` 读取。但项目尚不宜直接视为“生产发布完成”：真实 GPU、声卡和升级回退场景仍需要发布前人工验收。
...
- 直播单句生成失败自动重连并最多尝试3次；仍失败时跳过该句并维持后续缓存和播放。
- 当前句播放完成后插播预设话术。
...
最终结果：**99 passed，1 warning，1.75s**。

剩余警告是 Starlette TestClient/httpx 兼容弃用提示。FastAPI 生命周期已迁移到 lifespan。
...
关键播放状态专项测试（暂停时继续缓存、停止后重启、随机性模式和后端随机性）共13项，连续运行3轮均通过。
...
1. 代码已推送到远程 `source` 分支，但尚未配置分支保护和自动 CI。
2. WebSocket 已支持单句内短间隔重连重试，但没有跨句、长时间断网的指数退避恢复策略。
3. `client/app.py` 过大，状态机、UI、队列与业务逻辑高度耦合。
4. 音色仓库缺少删除、配额、重复文件清理和数据库维护功能。
5. Qwen prompt cache 未设置明确容量上限。
6. 上传只限制大小（50 MB），缺少可靠的媒体解码验证和 MIME 校验。
7. 完整安装默认指向固定 D 盘路径且缺少路径选择；便携 ZIP 更适合当前分发方式，但尚未形成单一标准构建入口。
8. 更新包有 SHA256 校验但没有数字签名。
9. `releases/` 和历史构建产物较多，缺少自动保留策略。
...
1. 默认关闭公网入口或强制安全认证，重点审查 7860 启动器与 WebSocket 鉴权。
2. 在两台全新 Windows 电脑验证 0.9.12 完整便携包、用户数据保留、升级与回退。
3. 建立 GitHub CI 门禁并保护 `source` 分支。
4. 拆分 `client/app.py`，为缓存调度与播放状态机补充基于事件序列的测试。
5. 做真实 GPU 2–4 小时压力测试，记录 RTF、失败率、显存和缓存水位。
